package com.store.cloud.product.controller.admin;
...
import com.store.cloud.product.dto.SkuDTO;
import com.store.cloud.product.dto.SpuDTO;
import com.store.cloud.product.dto.SpuPageSearchDTO;
import com.store.cloud.product.model.SpuExtension;
import com.store.cloud.product.service.*;
import com.store.cloud.api.product.vo.*;
...
public class SpuController {
    ...
    private void spuUpdateStatus(SpuDTO spu) {
        ...
        if (StrUtil.isNotBlank(error)) {
            throw new StorecloudException(error);
        }
    ...
    private void spuBatchUpdateStatus(SpuDTO spu) {
        ...
        if (CollUtil.isEmpty(spuList)) {
            throw new StorecloudException("您选择的商品信息有误，请刷新后重试");
        }
        ...
        if (CollUtil.isEmpty(spuIds)) {
            throw new StorecloudException("您所选择的商品中没有符合操作条件的商品");
        }
        spuService.batchRemoveSpuCacheBySpuId(spuIds);
        if (errorList.size() > 0) {
            throw new StorecloudException("商品id为：" + errorList.toString() + "的" + errorList.size() + "件商品不符合操作条件");
        }
        spuService.changeSpuStatus(spu.getSpuId(), spu.getStatus());
        spuService.removeSpuCacheBySpuId(spu.getSpuId());
    }
    ...
    private void checkSaveOrUpdateInfo(SpuDTO spuDTO) {
        if (!Objects.equals(Constant.PLATFORM_SHOP_ID, AuthUserContext.get().getTenantId()) && Objects.isNull(spuDTO.getShopCategoryId())) {
            throw new StorecloudException("店铺分类不能为空");
        }
        if (Objects.isNull(spuDTO.getCategoryId())) {
            throw new StorecloudException("平台分类不能为空");
        }
