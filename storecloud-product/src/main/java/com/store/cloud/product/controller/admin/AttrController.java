...
import com.store.cloud.product.constant.SearchType;
import com.store.cloud.product.dto.AttrDTO;
import com.store.cloud.product.model.Attr;
import com.store.cloud.product.model.AttrValue;
import com.store.cloud.product.service.AttrService;
...
public class AttrController {
    ...
    public ServerResponseEntity<Void> save(@Valid @RequestBody AttrDTO attrDTO) {
        if (Objects.equals(Constant.PLATFORM_SHOP_ID, AuthUserContext.get().getTenantId()) && Objects.isNull(attrDTO.getAttrType())) {
            throw new StorecloudException("属性类型不能为空");
        }
    ...
    private void checkAttrInfo(AttrDTO attrDTO) {
        ...
        if (CollUtil.isEmpty(attrDTO.getCategoryIds())) {
            throw new StorecloudException("关联分类不能为空");
        }
        if (Objects.isNull(attrDTO.getSearchType())) {
            throw new StorecloudException("搜索属性不能为空");
        }
    }
