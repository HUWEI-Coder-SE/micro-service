...
import com.store.cloud.api.product.bo.EsAttrBO;
import com.store.cloud.api.product.bo.EsProductBO;
import com.store.cloud.api.product.vo.CategoryVO;
import com.store.cloud.api.product.vo.SpuAttrValueVO;
...
import com.store.cloud.product.mapper.SpuMapper;
import com.store.cloud.product.service.*;
import com.store.cloud.api.product.vo.SpuVO;
import com.store.cloud.product.model.Spu;
import com.store.cloud.product.model.SpuAttrValue;
import com.store.cloud.product.model.SpuDetail;
...
public class SpuServiceImpl implements SpuService {
    ...
    public void changeSpuStatus(Long spuId, Integer status) {
        ...
        if (!Objects.equals(status, StatusEnum.ENABLE)) {
            ...
            if (!imgRes.isSuccess()) {
                throw new StorecloudException("服务异常");
            }
        }
    }
    ...
    public void deleteById(Long spuId) {
        ...
        if(Objects.isNull(spuVO) || Objects.equals(spuVO.getStatus(), StatusEnum.DELETE.value())){
            throw new StorecloudException("商品不存在或者已被删除！");
        }
        // 删除商品、sku信息(逻辑删除)
