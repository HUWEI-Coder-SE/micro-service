...
const util = {
  ...
  getUserInfo: () => {
    ...
    return new Promise((resolve, reject) => {
      http.request({
        url: '/storecloud_user/user/ma/user_detail_info',
        method: 'GET',
        dontTrunLogin: true
      }).then((res) => {
        resolve(res)
  ...
  refreshToken: () => {
    ...
    if (refreshToken && expiresTimeStamp && expiresTimeStamp < new Date().getTime()) {
      ...
      const params = {
        url: '/storecloud_auth/ua/token/refresh',
        method: 'POST',
        isRefreshing: true,
        data: {
          refreshToken
  ...
  getAllinpayConfig: () => {
    const params = {
      url: '/storecloud_admin/ua/app/sys_config/pay_settlement_type',
      method: 'GET'
    }
    http.request(params).then(res => {
      if (res) {
  ...
  getAllinpayMember: () => {
    const params = {
      url: '/storecloud_payment/allinpay/member/get_user_info',
      method: 'GET',
  ...
  createAllinpayMember: () => {
    const params = {
      url: '/storecloud_payment/allinpay/member/create_member',
      method: 'POST'
    }
    http.request(params)
  },
  ...
  loginByCode: (code) => {
    const params = {
      login: true,
      url: uni.getStorageSync('cloudAppType') === AppType.MINI ? '/storecloud_auth/ua/social/ma' : '/storecloud_auth/ua/social/mp',
      method: 'POST',
      data: code
    }
    http.request(params).then(res => {
  ...
  mioIoUpload: (imgItem, fn) => {
    const params = {
      url: '/storecloud_biz/ua/oss/upload_minio',
      filePath: imgItem,
      name: 'file',
      callBack: res => {
  ...
  area: async (address, name) => {
    ...
    const getListByPid = (pid) => {
      return new Promise((resolve) => {
        const params = {
          url: '/storecloud_order/area/list_by_pid',
          method: 'GET',
          data: {
            pid
          }
    ...
    if (!uni.getStorageSync('cloudProvArray')) {
      const params = {
        url: '/storecloud_order/area/list',
        method: 'GET',
        data: {}
      }
  ...
  getWebConfig: () => {
    const params = {
      url: '/storecloud_admin/ua/web_config/get_activity',
      method: 'GET',
      data: {
    ...
    return new Promise((resolve) => {
      http.request(params).then((res) => {
        if (res) {
          ...
          res.titleContentEn = res.titleContentEn || 'store'
  ...
  getTransactionId: (orderNumber) => {
    return new Promise(resolve => {
      http.request({
        url: `/storecloud_order/my_order/check_trade_managed/${orderNumber}`,
        method: 'GET'
      }).then(res => {
  ...
  confirmReceipt: orderNumber => {
    http.request({
      url: `/storecloud_order/my_order/receipt/${orderNumber}`,
      method: 'PUT'
    }).then(() => {
